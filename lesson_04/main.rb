...
def display_all
  ...
  station.show
  #station.trains.each { |train| puts "поезд №#{train.number} - тип: #{TYPE[train.typ_train]}, маршрут следования: #{train.route[0]} - #{train.route[-1]}, количество вагонов: #{train.carriages.size}"}
  end
end

def  create_station
  puts "Введите название новой станции"
  title = gets.chomp.downcase.capitalize!.to_s
